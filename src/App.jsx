...
function App() {
  ...
  return (
    <div className={`h-screen ${darkMode ? 'dark' : ''}`} >
      {user !== null && <Navbar selectedNavItem={selectedNavItem} onNavItemClick={handleNavItemClick} toggleDarkMode={toggleDarkMode} />}
      <div name="red container" className="text-default dark:text-dark dark:bg-gray-800 flex items-center justify-center" style={{ minHeight: `calc(100vh - ${NAVBAR_HEIGHT}px)` }}>
        {user === null ? ( // Check if the user is not signed in
          <SignIn auth={auth} />
        ) : (
          /* Display something else when the user is signed in */
          <div className="flex flex-wrap items-center justify-evenly text-default dark:text-dark  ">
